...
async function exportOrders(fromDate = null, toDate = null) {
  try {
    ...
    if (fromDate) {
      console.log(
        `Exporting orders from ${fromDate}${toDate ? ` to ${toDate}` : ""}...`
      );
    }
      ...
      try {
        ...
        for (const order of ordersResponse.items) {
          try {
            ...
            const transactions = await api.getOrderTransactions(
              order.entity_id
            );

            // Format and immediately write the order data
            const formattedOrderLines = csvWriter.formatOrderData(
              order,
              transactions
            );
            ...
            console.log(
              `Order ${order.increment_id} processed with ${formattedOrderLines.length} line items`
            );
          } catch (error) {
            console.error(
              `Error processing order ${order.increment_id}:`,
              error
            );
        ...
        console.log(
          `Progress: ${totalOrdersProcessed} orders (${totalLinesProcessed} lines) processed so far...`
        );
      } catch (error) {
    ...
    console.log(
      `Export completed successfully. Processed ${totalOrdersProcessed} orders with ${totalLinesProcessed} total line items.`
    );
  } catch (error) {
